-- Older pre-open row: should lose to the newer row below when selecting latest.
...
-- Newest row for the same date/slot.
INSERT INTO t_market_analyses (analysis_date, analysis_slot, scheduled_time_local, model, prompt_version, summary_text, raw_json, updated_at) VALUES
  ('2026-04-20', 'pre_tw_open', '07:30', 'gpt-5', 'v2', 'Latest pre-open summary', '{"k":"v"}', TIMESTAMP '2026-04-20 07:45:00');

-- Separate slot used by scheduler/admin slot-selection tests.
INSERT INTO t_market_analyses (analysis_date, analysis_slot, scheduled_time_local, model, prompt_version, summary_text, raw_json, updated_at) VALUES
  ('2026-04-20', 'us_close', '05:00', 'gpt-5', 'v1', 'Post close summary', '{}', TIMESTAMP '2026-04-20 05:00:00');

-- Active/inactive groups let tests prove test-only mode suppresses groups.
...
-- U_TEST is the only active test account in fixtures.
